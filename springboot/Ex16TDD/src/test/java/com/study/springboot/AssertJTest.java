...
public class AssertJTest {
    ...
    void test2(){
        ...
        //when
        //then
        Assertions.assertThat(list).contains("1"); //포함하면 통과
        Assertions.assertThat(list).isNotEmpty(); //비어있지 않으면 통과
        Assertions.assertThat(list).startsWith("1"); //"1"로 시작하면 통과
...
//        Assertions.assertThat(user1).isEqualTo(user2); //객체 주소 비교
    ...
    void test5() {
        ...
        // when, then
        Assertions.assertThatThrownBy(() -> input.charAt(input.length()+1))
    ...
    public void test8(){
        ...
        //Soft Assertion
        //assertj기능중 하나로서
        //동시에 여러개의 테스트를 할 때 중간에 에러가나도 멈추지 않고,
        //   모든 검사를 실행하고 결과를 보여줍니다.
