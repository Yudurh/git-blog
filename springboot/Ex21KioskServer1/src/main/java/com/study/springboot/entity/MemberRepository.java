...
public interface MemberRepository extends JpaRepository<MemberEntity, Long> {
    ...
    //1.확장함수 : 기본적인 함수만 쓰고, 복잡하면 NativeSQL을 사용하는 게 좋다.
    //2. JPQL : Java Persistence Query Language, JPA에서 지원하는 쿼리.
    //3. Native SQL : 표준 SQL을 지원한다.

    //find
    //private String user_id; 언더바는 생략된다. 단어와 단어는 대문자로 시작한다.
    ...
    //1. 확장함수 없음. 기본함수 save, delete 사용하면 됨.
    //2. JPQL
    //3. Native SQL(Query) : insert, update, delete문은 @Modifying, @Transactional을 써줘야 됨.
    // @Modifying : 수정하는 쿼리라는 것을 알려줌.
    // @Transactional : DB 트랜잭션(하나의 동작)에서 자동 commit(물리적 저장)을 의미함.
    @Modifying
    @Transactional
    @Query(nativeQuery = true,
