...
//    삭제(액션) 후에 JS Alert를 띄운후, redirect/replace(페이지이동)하기
    @GetMapping("/delete2")
    @ResponseBody
    public String delete2(@RequestParam String index, Model model){
        memberList.remove( Integer.parseInt(index) );
    ...
    public String updateAction(@RequestParam String index,
        ...
        Member member = new Member();
        member.setName(inputName);
        member.setName(inputPw);
        member.setName(inputEmail);
